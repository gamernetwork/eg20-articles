				...
				FB.getLoginStatus(
					function(response)
					{
						if(response.status === 'connected')
						{
							...
							var url = "facebook-transfer.php?token="+accessToken+"&anchor="+args.anchor;
						...
						else
						{
							FB.login(
								...
								{					
									...
									{					
										e.findElement().update(args.translations.working);
										var url = "facebook-transfer.php?token="+response.authResponse.accessToken+"&anchor="+args.anchor;
										document.location = url;
									}
									else
									{
...
CookieNotifier = Class.create(
{
	initialize: function(options)
	{
		...
		if(this.sid == 1 && !readCookie("cookie-message-dismissed"))
		{
			this.draw();			
		}
	},
...
/*
playback timings (ms):
  load_resource: 107.964
  LoadShardBlock: 86.018 (3)
  exclusion.robots.policy: 0.174
  PetaboxLoader3.datanode: 76.882 (4)
  PetaboxLoader3.resolve: 76.919
  exclusion.robots: 0.184
  CDXLines.iter: 15.128 (3)
  esindex: 0.011
  RedisCDXSource: 5.276
  captures_list: 109.735
*/