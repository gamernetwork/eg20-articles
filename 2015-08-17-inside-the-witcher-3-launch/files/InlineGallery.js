var InlineGallery = Class.create({
	...
	{		
		...
		else
		{
			...
			this.frame.observe("click", this.next.bind(this));
	...
	{		
		if(!image["ad"])
		{
			...
			url = image["original-url"] + "/EG11/thumbnail/" + thumbW + "x" + thumbH;
	...
	gridclick: function(e)
	{
		...
		var max = {
			x: Math.min(
				thumbnail.getAttribute("data-original-width"),
				(viewport.width - buffer)
			),
			y: Math.min(
				thumbnail.getAttribute("data-original-height"),
		...
		var image = new Element(
			"img",
			{
				...
				"src": original + "/EG11/thumbnail/" + scale,
				"width": imgX,
				"height": imgY,
	...
	{	
		...
		{
			this.frame.update('<iframe frameborder="0" framepadding="0" framespacing="0" scrolling="no" width="300" height="250" src="https://web.archive.org/web/20150820230723/http://ads.eurogamer.net/delivery/afr.php?zoneid='+this.adzone+'" class="advert" style="display: none;">&nbsp;</iframe>');
		}
		else
		{		
			this.frame.update('<img src="' + this.images[index].url + '" alt="' + this.images[index].caption + '" style="display: none;"/>');
			$$("#"+this.target + " .frame > img").shift().observe("click", this.next.bind(this));
		}
...
document.observe(
	...
	function()
	{
		...
		{
			newWidth = parseInt($$("article")[0].getStyle("width"));
		...
		$$("article .illustrationPreview > img").each(
			function(image)
			{		
		...
		{
			image = "https://web.archive.org/web/20150820230723/http://images.eurogamer.net/" + e.target.getAttribute("data-uri");
...
/*
     FILE ARCHIVED ON 23:07:23 Aug 20, 2015 AND RETRIEVED FROM THE
...
playback timings (ms):
  ...
  CDXLines.iter: 20.513 (3)
  PetaboxLoader3.datanode: 138.643 (5)
  RedisCDXSource: 1.165
  captures_list: 84.941
  load_resource: 373.994
  exclusion.robots.policy: 0.436
  PetaboxLoader3.resolve: 209.038 (2)
  exclusion.robots: 0.457
*/