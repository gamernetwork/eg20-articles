...
> \
	<p class='details'> \
		...
		<span class='verified'>%%VERIFIED-TITLE%%, <a href='https://web.archive.org/web/20121017094750/http://www.gamesindustry.biz/resources/directory/company/%%VERIFIED-COMPANY-TAG%%'>%%VERIFIED-COMPANY%%</a></span>\
	...
	scroll: function()
	{
		...
		{
			// let's load every remaining comment.
			this.options.limit = this.options.count;
			this.load();
		}
	},
	...
	{	
		if(i < this.options.comments.length)
		{
			...
			{
				this.options.comments[i].a = "https://web.archive.org/web/20121017094750/http://images.eurogamer.net/" + this.options.comments[i].a + "/EG11/thumbnail/30x30";
			}
			else
			{
	...
	spinner: function()
	{
		$(this.options.target).insert(new Element("li", { "class": "loading" }).update("<span>" + this.lang.loading + " <img src='img/EurogamerPage/spinner-pac-man.gif' alt=''/></span>"));
	},
