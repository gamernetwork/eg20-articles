...
> \
	<p class='details'> \
		...
		%%MOD%% \
		<span class='verified'>%%VERIFIED-TITLE%%, <a href='https://web.archive.org/web/20150820230717/http://www.gamesindustry.biz/resources/directory/company/%%VERIFIED-COMPANY-TAG%%'>%%VERIFIED-COMPANY%%</a></span>\
		<a href='profiles/%%USERNAME%%/comments/%%AID%%' class='timestamp'>%%TIMESTAMP%%</a> \
	</p> \
	<div class='post'> \
		<span id='post-%%ID%%'>%%POST%%</span> \
		<span> \
			<a href='#' class='tool reply' rel='nofollow'><i class='icon-share-alt'></i> " + this.lang.reply + "</a> \
			<span class='karma'> \
				<span id='karma-%%ID%%' data-id='%%ID%%' class='score'>%%KARMA%%</span> \
				<span class='total'>/%%TOTAL%%</span> \
				<a href='#' data-id='%%ID%%' class='karma down' rel='nofollow'>-</a> \
				<a href='#' data-id='%%ID%%' class='karma up' rel='nofollow'>+</a> \
			</span> \
			<a href='comment_edit.php?id=%%ID%%&action=edit' class='tool edit' rel='nofollow'><i class='icon-pencil'></i> " + this.lang.edit + "</a> \
			<a href='comment_edit.php?id=%%ID%%&action=delete' class='tool delete' rel='nofollow'><i class='icon-remove'></i> " + this.lang.del + "</a> \
			<a href='community.php?action=view-scorers&cid=%%ID%%' data-id='%%ID%%' class='tool scorers' rel='nofollow'><i class='icon-user'></i> Scorers</a> \
			%%BANBUTTON%% \
		</span> \
	</div> \
</li>";
	...
	{	
		if(i < this.options.comments.length)
		{
			...
			{
				this.options.comments[i].a = "//web.archive.org/web/20150820230717/http://images.eurogamer.net/" + this.options.comments[i].a + "/EG11/thumbnail/28x28/quality/90/format/jpg";
			}
			else
			{
			...
			temp = this.swap(temp, /\%\%MOD\%\%/g, this.options.comments[i].mod ? '<span class="moderator">' + this.lang.moderator + '</span>' : false);
			temp = this.swap(temp, /\%\%MY-SCORE\%\%/g, this.options.comments[i].s);
			temp = this.swap(temp, /\%\%TOTAL\%\%/g, this.options.comments[i].total);

			temp = this.swap(
				temp,
				/\%\%BANBUTTON\%\%/g,
				this.options.ismoderator ?
					"<a href='#' onclick='if(confirm(\"Really ban this user?\")){document.location=\"forum_moderation.php?action=gi&username=" + this.options.comments[i].u + "&id=" + this.options.comments[i].uid + "&reason=Comments+one-click+ban\";}return false;' class='tool ban' rel='nofollow'><i class='icon-legal'></i> " + this.lang.ban + "</a>"
					:
					false
			);
		...
		if(response.status == "OK" && response.id > 0)
		{
			...
			if(ga)
			{
				ga(
					...
					this.article.slug,
					{
						'dimension1': this.article.type,
						'dimension2': this.article.topics,
						'dimension3': this.article.platforms,
						'dimension4': this.article.author
					}
				);
			}
